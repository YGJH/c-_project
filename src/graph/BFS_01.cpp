...
void solve() {
    ...
    tmp.x = 0 , tmp.y = 0 , tmp.ste = 0 , tmp.pattern = che[0][0];
    que.push_back(tmp);
    NODE now ;
    while(!que.empty()) {
        now = que.front();
        ...
        vis[now.y][now.x] = 1;
        // cerr << now.y << ' ' << now.x << ' ' << now.ste << endl;
        for(i = 0 ; i < 4 ; i++) {
            tmp2.x = now.x + xx[i];
            tmp2.y = now.y + yy[i];
            if(tmp2.x < m && tmp2.y < n && tmp2.y >= 0 && tmp2.x >= 0) {
                if(vis[tmp2.y][tmp2.x] == 0){
                    tmp2.pattern = che[tmp2.y][tmp2.x];
                    if(tmp2.pattern != now.pattern) {
                        tmp2.ste = now.ste + 1;
                        que.push_back(tmp2);
                    }
                    else{
