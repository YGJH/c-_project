...
struct Scc{
  ...
  void solve(){
    ...
    vector<vector<int>> new_edge(2e5+5);
    int w[MXN];
    int vl[MXN];
    queue<int> Q;
    vector<int> dp(MXN);
void solve() {
    ...
    for(auto &c : edge) {
        ...
        c.first--;c.second--;
        sc.addEdge(c.first , c.second);
    }
    sc.solve();
    for(int i = 0 ; i < n ; i++) vl[sc.bln[i]] += w[i];
    for(auto &a : edge) {
        if(sc.bln[a.first] != sc.bln[a.second]) {
            deg[sc.bln[a.second]]++;
            new_edge[sc.bln[a.first]].pb(sc.bln[a.second]);
        }
    }

    int ans = 0;
    for ( int i = 0 ; i < sc.nScc ; i++) {
        if(!deg[i]) Q.push(i),dp[i] = vl[i];
    }
    while(!Q.empty()) {
        int u = Q.front(); Q.pop();
        ans = max(dp[u] , ans);
