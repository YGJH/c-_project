...
int a[200000]; // damage = value
int b[200000]; // cost = 耐重
vector<int> dp(200000, INT32_MAX);
void solve() {
  int h, n;
  re(h), re(n);
  int tmp = INT32_MIN;
  ...
  for (int i = 0; i < n; i++) {
    for (int j = a[i]; j <= 20000; j++) {
            ...
            dp[j] = mmin(dp[j], dp[j - a[i]] + b[i]);// 0 , INT64_MAX , > INT64_MAX , b[i]
    }
  }
  int ans = INT32_MAX;
  for (int i = h; i <= 200000 ; i++) {
        if(dp[i])
        ans = mmin(ans,dp[i]);
  }
  wr(ans);
